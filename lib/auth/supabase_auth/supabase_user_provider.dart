...
class BinaSystemSupabaseUser extends BaseAuthUser {
  BinaSystemSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> binaSystemSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = BinaSystemSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}

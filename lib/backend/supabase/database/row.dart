...
T? _supaDeserialize<T>(dynamic value) {
  ...
  switch (T) {
    ...
    case const (Uint8List):
      ...
      // 2. PostgreSQL hex format: \x followed by hex pairs
      // 3. Already decoded Uint8List
      // 4. List of integers
      if (value is String) {
        try {
          // Check for PostgreSQL hex format (starts with \x)
          if (value.startsWith('\\x') || value.startsWith(r'\x')) {
            final hexString = value.substring(2);
            // Decode hex string - the hex represents a JSON array string like "[255,216,...]"
            final bytes = <int>[];
            ...
            // The decoded bytes form a string like "[255,216,255,...]"
            final decodedString = String.fromCharCodes(bytes);
            // Check if it's a JSON array
            if (decodedString.startsWith('[') && decodedString.endsWith(']')) {
              final jsonList = json.decode(decodedString) as List<dynamic>;
              return Uint8List.fromList(jsonList.cast<int>()) as T?;
            }
            // Otherwise return the hex-decoded bytes directly
        ...
        return Uint8List.fromList(value.cast<int>()) as T?;
      }
      return null;
    default:
      return value as T;
